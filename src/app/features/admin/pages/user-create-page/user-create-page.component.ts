...
export class UserCreatePageComponent {
  userForm: FormGroup;
  beltOptions = ['blanco', 'amarillo', 'naranja', 'verde', 'azul', 'marrón', 'negro'];
...
submit(): void {
  if (this.userForm.valid) {
    ...
    if (this.editMode && this.userId !== null) {
      this.http.put(`http://localhost:3000/students/${this.userId}`, userData).subscribe(() => {
        this.snackBar.open('Usuario actualizado correctamente', 'Cerrar', { duration: 3000 });
        this.router.navigate(['/admin/students']);
      });
    } else {
      this.http.post('http://localhost:3000/students', userData).subscribe(() => {
